...
var getWeeklyData = () => {
  return new Promise(function(resolve, reject) {
    client.query(
      `select a.name, b.coin_id, b.price, to_date(time_stamp, 'mm/dd/yy HH') as date
      from coin a
      inner join price_history b on a.id = b.coin_id
      where to_date(time_stamp, 'mm/dd/yy HH')
...
var getYearData = () => {
  return new Promise(function(resolve, reject) {
    client.query(
      ...
      (
        select a.name, b.coin_id, to_char(to_date(time_stamp, 'mm/dd/yy HH'), 'Month') as monthName, avg(price) over (partition by to_char(to_date(time_stamp, 'mm/dd/yy HH'), 'Month')) as avgMonthPrice
        from coin a
        inner join price_history b on a.id = b.coin_id
        where to_date(time_stamp, 'mm/dd/yy HH')
